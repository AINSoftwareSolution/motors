...
const OurBrand = () => {
  return (
    <div className="container mx-auto p-4 pt-6 md:p-6 lg:px-12 mt-4 mb-4 text-center dark:bg-gray-900 overflow-hidden">
      <h2 className="text-black dark:text-white text-3xl sm:text-4xl font-bold mt-5">Vehicle Collections</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mt-5 mb-4">
        {CarBrands.map((brand, index) => (
          ...
          >
            ...
            <h3 className="mt-4 font-bold text-md sm:text-lg text-black dark:text-white">{brand.name}</h3>
            <p className="mt-2 text-gray-600 dark:text-gray-400 text-sm sm:text-base">{brand.p}</p>
          </div>
        ))}
      </div>
    </div>
